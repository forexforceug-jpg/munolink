...
export default function Connections({ navigation }) {
  ...
  const quickCategories = [
    { icon: 'storefront-outline', title: 'Shops Near You', desc: 'Discover local stores', color: C.primary, bg: C.greenBg },
    { icon: 'briefcase-outline', title: 'Service Providers', desc: 'Find professionals', color: '#1976D2', bg: C.blueBg },
    { icon: 'pricetag-outline', title: 'Deals For You', desc: 'Exclusive offers', color: C.warning, bg: C.orangeBg },
    { icon: 'star-outline', title: 'Top Rated', desc: 'Best businesses', color: C.purple, bg: C.purpleBg },
    { icon: 'calendar-outline', title: 'Events Near You', desc: 'Upcoming events', color: C.danger, bg: C.redBg },
  ];
  ...
  const requireAuth = (action) => { if (!user) { setShowAuthPopup(true); return false; } action(); return true; };

  if (pageLoading) return (<View style={styles.container}><View style={styles.header}><View style={styles.headerLeft}><Image source={LogoImage} style={styles.headerLogo} resizeMode="contain" /></View></View><View style={styles.loadingContainer}><ActivityIndicator size="large" color={C.accent} /><Text style={styles.loadingText}>Loading...</Text></View></View>);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          <TouchableOpacity style={styles.headerIcon} onPress={() => setShowNotifDropdown(!showNotifDropdown)}><Ionicons name="notifications-outline" size={24} color={C.text} /><View style={styles.notifBadge}><Text style={styles.notifBadgeText}>5</Text></View></TouchableOpacity>
          <TouchableOpacity onPress={() => setShowProfileDropdown(!showProfileDropdown)}><View style={styles.profilePic}><Ionicons name="person" size={20} color={C.white} />{user && <View style={styles.onlineDot} />}</View></TouchableOpacity>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.searchContainer}>
          ...
          <View style={[styles.searchRow, isSearching && styles.searchRowActive]}>
            <View style={styles.searchBar}><Ionicons name="search-outline" size={18} color={C.muted} /><TextInput style={styles.searchInput} value={searchQuery} onChangeText={fetchSuggestions} placeholder="Search..." placeholderTextColor={C.muted} onFocus={()=>{if(searchQuery.length<2&&!isSearching){setSuggestions(searchHistory.map(i=>({text:i,type:'history'})));setShowSuggestions(true);}}} onSubmitEditing={()=>{if(searchQuery.length>=2)runSearch(searchQuery);}} />{searchQuery.length>0&&<TouchableOpacity onPress={handleClearSearch}><Ionicons name="close-circle" size={18} color="#CCC" /></TouchableOpacity>}</View>
        ...
        {isSearching ? (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.recentScroll}>{featuredShops.map(shop=>(<TouchableOpacity key={shop.id} style={styles.recentCard} onPress={()=>navigation.navigate('ShopProfile',{shopId:shop.id})}><View style={styles.recentAvatar}><Ionicons name="storefront" size={22} color={C.accent} />{shop.is_open&&<View style={styles.recentOnlineDot}/>}</View><Text style={styles.recentName} numberOfLines={1}>{shop.name}</Text><Text style={styles.recentCategory}>{shop.category}</Text></TouchableOpacity>))}</ScrollView>
        </>) : (
      ...
      <View style={styles.bottomNav}>
        <TouchableOpacity style={styles.navItem} onPress={()=>requireAuth(()=>navigation.navigate('MyWallet'))}><Ionicons name="wallet-outline" size={22} color={C.muted}/><Text style={styles.navLabel}>Wallet</Text></TouchableOpacity>
        <TouchableOpacity style={styles.navItem} onPress={()=>requireAuth(()=>navigation.navigate('MyOrders'))}><Ionicons name="calendar-outline" size={22} color={C.muted}/><Text style={styles.navLabel}>Orders</Text></TouchableOpacity>
        <TouchableOpacity style={styles.navItem}><Ionicons name="people" size={22} color={C.accent}/><Text style={[styles.navLabel,styles.navLabelActive]}>Connections</Text></TouchableOpacity>
        <TouchableOpacity style={styles.navItem} onPress={()=>requireAuth(()=>navigation.navigate('Messages'))}><Ionicons name="chatbubbles-outline" size={22} color={C.muted}/><View style={styles.navBadge}><Text style={styles.navBadgeText}>3</Text></View><Text style={styles.navLabel}>Messages</Text></TouchableOpacity>
        <TouchableOpacity style={styles.navItem} onPress={()=>setShowProfileDropdown(!showProfileDropdown)}><Ionicons name="person-outline" size={22} color={C.muted}/><Text style={styles.navLabel}>Account</Text></TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  categoryCard: { width: 140, backgroundColor: C.white, borderRadius: 16, padding: 14, marginRight: 10, elevation: 2, borderWidth: 1, borderColor: C.border },
  categoryIcon: { width: 44, height: 44, borderRadius: 14, justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
  categoryTitle: { fontSize: 13, fontWeight: '700', color: C.text, marginBottom: 2 },
  categoryDesc: { fontSize: 10, color: C.muted },
  ...
  notifDropdown: { position: 'absolute', top: 95, right: 16, width: 300, backgroundColor: C.white, borderRadius: 16, elevation: 8, zIndex: 200, borderWidth: 1, borderColor: C.border, overflow: 'hidden' },
  ...
  profileDropdown: { position: 'absolute', top: 95, right: 16, width: 220, backgroundColor: C.white, borderRadius: 16, elevation: 8, zIndex: 200, borderWidth: 1, borderColor: C.border, overflow: 'hidden' },
