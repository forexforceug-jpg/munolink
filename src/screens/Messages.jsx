...
export default function Messages({ navigation, route }) {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      if (partnerIds.length > 0) { 
        const { data: users } = await supabase.from('users').select('id, full_name').in('id', partnerIds); 
        if (users) users.forEach(u => { partnerMap[u.id] = u.full_name || 'User'; }); 
        const { data: shops } = await supabase.from('shops').select('id, name, owner_id').in('owner_id', partnerIds); 
  ...
  const handleSend = async () => {
    if (!messageText.trim() || !selectedChat || !isMounted.current) return;
  ...
  const filteredConversations = conversations.filter(c => { 
    const m = c.name?.toLowerCase().includes(searchQuery.toLowerCase()); 
    return activeFilter === 'unread' ? m && c.unread > 0 : m; 
  });
  
  const unreadCount = conversations.filter(c => c.unread > 0).length;
  const totalCount = conversations.length;

  // CHAT VIEW - Bottom navbar hidden
  if (selectedChat) {
    return (
        ...
        <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 90 : 0}>
          ...
          <FlatList 
            ...
            renderItem={({ item }) => (
              <View style={[styles.messageRow, item.from === 'me' ? styles.messageRowMe : styles.messageRowThem]}>
                {item.from === 'them' && <View style={styles.messageAvatar}><Ionicons name="person" size={16} color={C.accent} /></View>}
                <View style={[styles.messageBubble, item.from === 'me' ? styles.messageBubbleMe : styles.messageBubbleThem]}>
                  <Text style={styles.messageText}>{item.text}</Text>
                  <View style={styles.messageFooter}>
                    <Text style={styles.messageTime}>{item.time}</Text>
                    {item.from === 'me' && <Ionicons name="checkmark-done" size={12} color={C.accent} />}
                  </View>
                </View>
              </View>
            )} 
          ...
          {showQuickShare && (
            <View style={styles.quickShareRow}>
              {[
                { icon: 'images-outline', label: 'Gallery', color: C.success },
                { icon: 'camera-outline', label: 'Camera', color: '#2196F3' },
                { icon: 'location-outline', label: 'Location', color: C.danger },
                { icon: 'document-outline', label: 'Document', color: C.warning },
                { icon: 'person-add-outline', label: 'Contact', color: C.purple }
              ].map((item) => (
                <TouchableOpacity key={item.label} style={styles.quickShareItem} onPress={() => { setShowQuickShare(false); if (item.label === 'Location') setMessageText('📍 My location: Jinja City, Uganda'); }}>
          ...
          <View style={styles.composer}>
            <TouchableOpacity style={styles.composerPlus} onPress={() => setShowQuickShare(!showQuickShare)}>
              <Ionicons name={showQuickShare ? 'close' : 'add'} size={24} color={C.accent} />
            </TouchableOpacity>
  ...
  // MAIN MESSAGES LIST VIEW - With bottom navbar
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        ...
        {loading ? (
          <View style={styles.loadingContainer}><ActivityIndicator size="large" color={C.accent} /></View>
        ) : (
          <FlatList 
        ...
        <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('Connections')}>
          <Ionicons name="create-outline" size={24} color={C.white} />
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  promoCard: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: C.lightBg, borderRadius: 16, padding: 16, marginTop: 8, marginBottom: 8 },
  ...
  fab: { position: 'absolute', bottom: 100, right: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: C.accent, justifyContent: 'center', alignItems: 'center', elevation: 6 },
  chatHeader: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingTop: 12, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: C.border, gap: 10, backgroundColor: C.white },
