    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('Account')}><View style={styles.profilePic}><Ionicons name="person" size={20} color={C.white} /><View style={styles.onlineDot} /></View></TouchableOpacity>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[C.accent]} />}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statCard}><Ionicons name="receipt-outline" size={16} color={C.accent} /><Text style={styles.statValue}>{orders.length}</Text><Text style={styles.statLabel}>Total</Text></View>
          <View style={styles.statCard}><Ionicons name="cash-outline" size={16} color={C.accent} /><Text style={styles.statValue}>UGX {totalSpent.toLocaleString()}</Text><Text style={styles.statLabel}>Spent</Text></View>
          <View style={styles.statCard}><Ionicons name="pricetag-outline" size={16} color={C.accent} /><Text style={styles.statValue}>UGX {totalSaved.toLocaleString()}</Text><Text style={styles.statLabel}>Saved</Text></View>
        </View>

        <View style={styles.searchRow}><View style={styles.searchBar}><Ionicons name="search-outline" size={18} color={C.muted} /><TextInput style={styles.searchInput} value={searchQuery} onChangeText={setSearchQuery} placeholder="Search orders..." placeholderTextColor={C.muted} />{searchQuery.length > 0 && <TouchableOpacity onPress={() => setSearchQuery('')}><Ionicons name="close-circle" size={18} color="#CCC" /></TouchableOpacity>}</View></View>

        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabsScroll}>{tabs.map(tab => (<TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.tabActive]} onPress={() => setActiveTab(tab)}><Text style={[styles.tabText, activeTab === tab && styles.tabTextActive]}>{tab}</Text></TouchableOpacity>))}</ScrollView>

        {loading ? (<View style={styles.loadingContainer}><Text style={styles.loadingText}>Loading orders...</Text></View>) : filteredOrders.length === 0 ? (<View style={styles.emptyState}><Ionicons name="receipt-outline" size={48} color="#CCC" /><Text style={styles.emptyTitle}>No orders found</Text></View>) : filteredOrders.map((order, index) => (
          <TouchableOpacity key={index} style={styles.orderCard} onPress={() => navigation.navigate('OrderDetails', { order })} activeOpacity={0.7}>
            <View style={[styles.statusIconCircle, { backgroundColor: order.statusBg }]}><Ionicons name={getStatusIcon(order.status)} size={22} color={order.statusColor} /></View>
            <View style={styles.orderInfo}>
              <View style={styles.orderTopRow}><Text style={styles.orderId}>{order.id}</Text><Ionicons name="chevron-forward" size={16} color="#CCC" /></View>
              <Text style={styles.orderShop}>{order.shop}</Text>
              <View style={styles.orderMeta}><Ionicons name="location-outline" size={10} color={C.muted} /><Text style={styles.orderMetaText}>{order.distance} · {order.location}</Text></View>
              <Text style={styles.orderDate}>{order.date}</Text>
            </View>
            <View style={styles.orderRight}>
              <View style={[styles.statusBadge, { backgroundColor: order.statusBg }]}><Text style={[styles.statusText, { color: order.statusColor }]}>{order.status}</Text></View>
      ...
      <View style={styles.bottomNav}>
        ...
        <TouchableOpacity style={styles.navItem}><Ionicons name="calendar" size={22} color={C.accent} /><Text style={[styles.navLabel, styles.navLabelActive]}>Orders</Text></TouchableOpacity>
