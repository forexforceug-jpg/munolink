...
export const AIBottomSheet: React.FC<AIBottomSheetProps> = ({
  ...
  // QUERY MARKETPLACE
  // ============================================================
  const queryMarketplace = useCallback(
    async (opp: Opportunity): Promise<MarketplaceData | null> => {
      try {
        ...
        const shopId = (opp as any).shopId;
        let sellerData: any = {};
        if (shopId) {
          const { data } = await supabase
            .from('shops')
          ...
          <Text key={`bold-${index}`} style={styles.aiTextLine}>
            {parts.map((part, i) => {
              ...
              return (
                <Text
                  key={`part-${i}`}
                  style={isBold ? styles.aiBoldText : styles.aiTextLine}
                >
                  {part}
                </Text>
              );
    ...
    async (query: string, opp: Opportunity): Promise<AIMessage> => {
      ...
      if (!marketData) {
        return {
          ...
          actions: [
            { type: 'view_seller', label: 'View Seller', data: { shopId: (opp as any).shopId } },
      ...
      if (
        ...
        return {
          ...
          text:
            ...
            `**Seller Rating:** ${
              marketData.rating > 0
                ? `${marketData.rating.toFixed(1)}⭐ (${marketData.reviewCount} reviews)`
                : 'New seller'
            }\n` +
            ...
            `• Current price is ${priceDiff > 0 ? '+' : ''}${Math.round(priceDiff)}% ${
              priceDiff < 0 ? 'below' : 'above'
            } average\n` +
            `• ${shopName} is ${
              marketData.rating > 4.0
                ? 'highly rated'
                : marketData.rating > 3.0
                ? 'reliable'
                : 'new'
            }\n\n` +
            `💡 **Verdict:** ${
              isGoodDeal
                ...
                : priceDiff < 0
                ? '🟡 Competitive pricing'
                : '🟠 Consider comparing with other options.'
            }`,
          actions: [
            { type: 'view_seller', label: 'View Seller', data: { shopId: (opp as any).shopId } },
            { type: 'see_similar', label: 'See Similar', data: { category: opp.category } },
      ...
      if (
        ...
        return {
          ...
          text:
            ...
            `**Price Difference:** ${priceDiff > 0 ? '+' : ''}${Math.round(priceDiff)}% ${
              priceDiff < 0 ? 'below' : 'above'
            } average\n` +
            `**Seller Rating:** ${
              marketData.rating > 0
            ...
            `**Availability:** ${marketData.inStock ? '✅ In Stock' : '❌ Out of Stock'}\n\n` +
            `**Value Score:** ${
              isGoodDeal ? '🟢 Excellent' : priceDiff < 0 ? '🟡 Good' : '🟠 Average'
            }\n\n` +
            `💡 **Recommendation:** ${
              isGoodDeal
                ? '✅ This is a great deal! Highly recommended.'
                : priceDiff < 0
      ...
      if (
        ...
        return {
          ...
          text:
            ...
            `${
              rating > 0
                ? `**Overall Rating:** ${rating.toFixed(1)} ⭐ (${reviewCount} reviews)\n\n`
                : '**No reviews yet**\n\n'
            }` +
            `**Seller Reputation:**\n` +
            `• ${shopName} has been rated by ${
      ...
      if (
        ...
        const estimatedDays =
          marketData.location === 'Kampala' || marketData.location === 'Jinja'
            ? '1-2 business days'
            : '2-4 business days';
        return {
          id: Date.now().toString(),
          type: 'ai',
          text:
            `🚚 **Delivery Information for ${productName}**\n\n` +
            `📍 **Location:** ${marketData.location}\n` +
            `📦 **Status:** ${marketData.inStock ? '✅ In Stock' : '❌ Out of Stock'}\n\n` +
            `**Estimated Delivery:**\n` +
            `• ${estimatedDays}\n\n` +
          ...
          actions: [
            { type: 'contact', label: 'Contact Seller', data: { shopId: (opp as any).shopId } },
            { type: 'view_seller', label: 'View Seller', data: { shopId: (opp as any).shopId } },
          ],
          timestamp: new Date().toISOString(),
        };
      ...
      if (
        ...
        return {
          ...
          type: 'ai',
          text:
            `🛡️ **Warranty & Returns for ${productName}**\n\n` +
            `⚠️ **I couldn't verify the exact warranty from the seller's listing.**\n\n` +
            `**What I can tell you:**\n` +
            `• ${shopName} has a ${
              marketData.rating > 4.0 ? 'premium' : 'standard'
            } seller reputation\n` +
            `• ${
              marketData.rating > 4.0
                ? 'Highly rated sellers typically offer better warranty terms'
                : 'Check with the seller for specific warranty details'
            }\n\n` +
            `💡 **Next steps:**\n` +
      ...
      if (
        ...
        return {
          ...
          type: 'ai',
          text:
            `💰 **Pricing Information for ${productName}**\n\n` +
            `**Current Price:** ${currency} ${price.toLocaleString()}\n` +
            `**Market Average:** ${currency} ${Math.round(avgPrice).toLocaleString()}\n` +
            `**Seller:** ${shopName}\n` +
            ...
            `💡 ${
              price > 500000
                ? 'Premium product with higher value'
                : 'Affordable option with good value'
            }`,
          actions: [
            { type: 'view_seller', label: 'View Seller', data: { shopId: (opp as any).shopId } },
            { type: 'see_similar', label: 'See Similar', data: { category: opp.category } },
      ...
      return {
        ...
        type: 'ai',
        text:
          `🤔 **Here's what I know about ${productName}**\n\n` +
          `**Overview:**\n` +
          `• ${productName}\n` +
          `• Category: ${opp.category || 'Uncategorized'}\n` +
          `• Price: ${currency} ${price.toLocaleString()}\n\n` +
          `**Seller:** ${shopName} ${
            marketData.rating > 0 ? `(${marketData.rating.toFixed(1)}⭐)` : ''
          }\n` +
          `**Location:** ${marketData.location}\n` +
          `**Status:** ${marketData.inStock ? '✅ In Stock' : '❌ Out of Stock'}\n\n` +
          `${
            Object.keys(opp.specifications || {}).length > 0
              ? `**Key Features:**\n${Object.entries(opp.specifications || {})
                  .slice(0, 3)
                  .map(([k, v]) => `• ${k}: ${v}`)
                  .join('\n')}\n\n`
              : ''
  ...
  const getInitialMessage = useCallback(() => {
    ...
    const shopName = (opportunity as any)?.shopName || 'the seller';
    const rating = opportunity?.rating || 0;

    let baseMessage = `👋 **Hi! I'm muno AI.**\n\nI can help you learn more about **${productName}** — this ${category} from ${shopName}${
      rating > 0 ? ` (${rating.toFixed(1)}⭐)` : ''
    } priced at ${currency} ${price.toLocaleString()}.`;
  ...
  const handleActionPress = (action: AIAction) => {
    ...
    switch (action.type) {
      ...
      case 'contact':
      case 'view_reviews':
        // Hook up to real navigation later
        break;
    }
  };
          ...
          {messages.map((message) => (
            <View
              ...
              style={[
                ...
                message.type === 'user'
                  ? styles.desktopUserWrapper
                  : styles.desktopAIWrapper,
              ]}
            >
              <View
                style={[
                  ...
                  message.type === 'user'
                    ? styles.desktopUserBubble
                    : styles.desktopAIBubble,
                ]}
              >
                {message.type === 'ai' && (
      ...
      >
        ...
        <SafeAreaView
          style={[
            ...
            { height: modalHeight, paddingBottom: bottomInset },
          ]}
        >
          ...
          {opportunity ? (
            <>
              ...
              >
                  ...
                  messages.map((message) => (
                    <View
                      ...
                      style={[
                        ...
                        message.type === 'user'
                          ? styles.mobileUserWrapper
                          : styles.mobileAIWrapper,
                      ]}
                    >
                      <View
                        style={[
                          ...
                          message.type === 'user'
                            ? styles.mobileUserBubble
                            : styles.mobileAIBubble,
                        ]}
                      >
                        {message.type === 'ai' && (
                        ...
                        <View style={styles.mobileMessageContent}>
                          {message.type === 'ai' ? (
                            ...
                            <Text style={styles.mobileUserText}>
                              {message.text}
                            </Text>
                          )}
                        </View>
                ...
                {isLoading && (
                  <View
                    style={[styles.mobileMessageWrapper, styles.mobileAIWrapper]}
                  >
                    <View
                      style={[styles.mobileMessageBubble, styles.mobileAIBubble]}
                    >
                      <Text style={styles.mobileAIIcon}>🔮</Text>
                      <View style={styles.mobileMessageContent}>
                        {renderTypingDots()}
...
const styles = StyleSheet.create({
  ...
  mobileAIBubble: {
    ...
    borderBottomLeftRadius: 4,
    borderWidth: 1,
    borderColor: 'rgba(74,125,255,0.25)',
  },
  mobileAIIcon: {
    fontSize: 18,
    marginRight: 10,
